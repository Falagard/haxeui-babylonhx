...
class StyleHelper {
    public static function apply(c:ComponentImpl, style:Style, w:Float, h:Float):Void {
        ...
        if (c.styleGraphics == null) {
            styleGraphics = new Graphics();
            c.styleGraphics = styleGraphics;
        } else {
            styleGraphics = c.styleGraphics;
        }
        styleGraphics.clear();
        ...
        if (style.backgroundImage != null) {
            Toolkit.assets.getImage(style.backgroundImage, function(imageInfo:ImageInfo) {
                ...
                if (c.bgImageGraphics == null) {
                    bgImageGraphics = new Graphics();
                    c.bgImageGraphics = bgImageGraphics;
                } else {
                    bgImageGraphics = c.bgImageGraphics;
                }
                bgImageGraphics.clear();
                
                var tile = TileCache.get(style.backgroundImage);
                if (tile == null) {
                    tile = TileCache.set(style.backgroundImage, Tile.fromBitmap(imageInfo.data));
                }
	...
	public static function drawRoundedRect(gfx:Graphics, x : Float, y : Float, w : Float, h : Float, radius : Float, nsegments = 0 ) {
		...
		if( nsegments == 0 ) {
            ...
            nsegments = Math.ceil(Math.abs(radius * com.babylonhx.math.Angle.FromDegrees(90).radians() / 4));
        }
		if ( nsegments < 3 ) nsegments = 3;
		//var angle = hxd.Math.degToRad(90) / (nsegments - 1);
        var angle = com.babylonhx.math.Angle.FromDegrees(90).radians() / (nsegments - 1);

		inline function corner(x, y, angleStart) {
		for ( i in 0...nsegments) {
			var a = i * angle + com.babylonhx.math.Angle.FromDegrees(angleStart).radians(); //hxd.Math.degToRad(angleStart);
